...
static func reserve_belt_slots(
	...
	for host in orbit_hosts:
		var available_distances: Array[float] = _collect_available_slot_distances(all_slots, host.node_id)
		var host_belts: Array[AsteroidBelt] = _generate_belts_for_host_from_distances(
			host,
			available_distances,
			stars,
			rng
		...
		for belt in host_belts:
			result.belts.append(belt)
			var reserved_slot_id: String = _find_best_reservation_slot_id(all_slots, belt)
			if not reserved_slot_id.is_empty():
				result.reserved_slot_ids.append(reserved_slot_id)
