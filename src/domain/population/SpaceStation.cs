...
public partial class SpaceStation : RefCounted
{
	...
	/// <summary>
	/// Hull displacement in tons for the detailed design system.
	/// Zero if detailed design has not been generated.
	/// </summary>
	public int HullTonnage;

	/// <summary>
	/// Detailed design result. Null if not generated.
	/// </summary>
	public StarGen.Domain.Population.StationDesign.DesignResult? DetailedDesign;

	/// <summary>
	/// Classification report. Null if not generated.
	/// </summary>
	...
	public Dictionary ToDictionary()
	{
		...
		data["hull_tonnage"] = HullTonnage;
		if (DetailedDesign != null)
		{
			data["detailed_design"] = DetailedDesign.ToDictionary();
			data["detailed_design_compact"] = DetailedDesign.ToCompactDictionary();
			data["detailed_design_spec_template"] = (int)DetailedDesign.Spec.Template;
			data["detailed_design_spec_hull"] = DetailedDesign.Spec.HullTons;
			data["detailed_design_spec_config"] = (int)DetailedDesign.Spec.Configuration;
			data["detailed_design_spec_auto_flags"] = (int)DetailedDesign.Spec.AutoFlags;
			data["detailed_design_spec_officer_ratio"] = DetailedDesign.Spec.OfficerRatio;
			data["detailed_design_catalog_version"] = DetailedDesign.CatalogVersion;
		}
	...
	public static SpaceStation FromDictionary(Dictionary data)
	{
		...
		station.HullTonnage = GetInt(data, "hull_tonnage", 0);
		if (data.ContainsKey("detailed_design") && data["detailed_design"].VariantType == Variant.Type.Dictionary)
		{
			station.DetailedDesign = StarGen.Domain.Population.StationDesign.DesignResult.FromDictionary(
				(Dictionary)data["detailed_design"]);
			if (data.ContainsKey("classification_report") && data["classification_report"].VariantType == Variant.Type.Dictionary)
			{
				station.ClassificationReport =
					StarGen.Domain.Population.StationDesign.Classification.ClassificationReport.FromDictionary(
						(Dictionary)data["classification_report"]);
			}
			else
			{
				station.ClassificationReport =
					StarGen.Domain.Population.StationDesign.Classification.ClassificationEvaluator.Evaluate(
						station.DetailedDesign);
			}
		}
		else if (data.ContainsKey("detailed_design_compact") && data["detailed_design_compact"].VariantType == Variant.Type.Dictionary)
		{
			station.DetailedDesign = StarGen.Domain.Population.StationDesign.DesignResult.FromCompactDictionary(
				(Dictionary)data["detailed_design_compact"]);
			station.ClassificationReport =
				StarGen.Domain.Population.StationDesign.Classification.ClassificationEvaluator.Evaluate(
					station.DetailedDesign);
		}
		else if (data.ContainsKey("detailed_design_spec_template"))
		{
			Dictionary compactDesign = new()
			{
				["catalog_version"] = GetString(data, "detailed_design_catalog_version", string.Empty),
				["spec"] = new Dictionary
				{
					["template"] = GetInt(data, "detailed_design_spec_template", (int)StarGen.Domain.Population.StationDesign.DesignTemplate.Custom),
					["hull_tons"] = GetInt(data, "detailed_design_spec_hull", station.HullTonnage),
					["configuration"] = GetInt(data, "detailed_design_spec_config", (int)StarGen.Domain.Population.StationDesign.HullConfiguration.Sphere),
					["auto_flags"] = GetInt(
						data,
						"detailed_design_spec_auto_flags",
						(int)StarGen.Domain.Population.StationDesign.AutoPopulateFlags.AllAuto),
					["officer_ratio"] = GetDouble(data, "detailed_design_spec_officer_ratio", 0.2),
				},
			};
	...
	private static double GetDouble(Dictionary data, string key, double fallback)
	{
		...
		if (value.VariantType == Variant.Type.String && double.TryParse((string)value, out double parsed))
		{
			return parsed;
		}
