...
public static partial class StationGenerator
{
		...
		SeededRng rng)
	{
		...
		if (spec.GenerateDetailedDesign)
		{
			StarGen.Domain.Population.StationDesign.DesignTemplate designTemplate =
				StarGen.Domain.Population.StationDesign.DesignMapping.DeriveTemplate(
					station.PrimaryPurpose, station.StationClass);

			int designHull = StarGen.Domain.Population.StationDesign.DesignMapping.DeriveHullTonnage(
				designTemplate,
				station.StationClass,
				station.Population);

			StarGen.Domain.Population.StationDesign.HullConfiguration designConfig =
				StarGen.Domain.Population.StationDesign.DesignMapping.DeriveHullConfiguration(
					station.StationType, designTemplate);

			station.HullTonnage = designHull;

			StarGen.Domain.Population.StationDesign.DesignSpec designSpec = new()
			{
				Template = designTemplate,
				HullTons = designHull,
				Configuration = designConfig,
				AutoFlags = StarGen.Domain.Population.StationDesign.AutoPopulateFlags.AllAuto,
			};
