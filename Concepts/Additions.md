...
**Selected for current hardening pass:** EcologyGenerator, ReligionGenerator, CivilisationEngine, ConlangGenerator, DiseaseSimulator, and EvoTechTree remain the active concept prototypes. The original atlas-access work lives on `codex/concept-atlas-fold-in`; the current determinism and dependency-chain rework is on `codex/concept-pipeline-hardening`. EcologyGenerator and ReligionGenerator are the closest to migration parity. CivilisationEngine, EvoTechTree, ConlangGenerator, and DiseaseSimulator now have native branch implementations, but they still require additional fidelity, prototype-parity review, and explicit human audit before prototype retirement.
...
| 6 | Speciation Pressure / Adaptive Landscape Simulator | Simulator | Prototype · EvoTechTree | Selection by biome, predator/prey, sexual selection, bottlenecks, isolation. Active hardening branch now has a deterministic species/sentience pipeline, but prototype-parity work is still in progress. |
...
| 9 | Disease & Epidemiology Simulator (Bio + Civ) | Simulator | Prototype · DiseaseSimulator | Pathogens, transmission, host range, virulence/transmissibility, immunity, population impact. Deterministic native port is underway, but richer prototype fidelity and tuning are still required. |
...
| 15 | Internal Politics / Power Bloc Simulator | Simulator | Prototype · CivilisationEngine | Elites, military, clergy, merchants, labor; influence, reform/reaction, coups, succession. Native port exists on the hardening branch, but deeper prototype parity is still required. |
| 16 | Religion / Belief System Generator | Generator | Prototype · ReligionGenerator | Cosmology, ritual intensity, clerical structure, moral axes, syncretism/schism, missionary vs insular. Closest to migration parity: deterministic seeded prototype already informs the native branch pipeline, with sentient-population input hardening underway. |
| 17 | Culture Generator (values, norms, aesthetics) | Generator | Prototype · CivilisationEngine | CivilisationEngine has culture sim. Native branch work now uses deterministic society state, but culture fidelity and audit depth still need review before retirement. |
| 18 | Language Family / Linguistics Generator | Generator | Prototype · ConlangGenerator | Proto-language, sound changes, branching families, naming conventions, loanwords. Native deterministic port is active on the hardening branch, but deeper family/history fidelity remains open. |
...
| 20 | Technology Diffusion & Innovation Model | Simulator | Prototype · CivilisationEngine | CivilisationEngine has tech tree. Branch work now seeds society outputs from sentient populations, but fuller simulation parity still belongs to the prototype backlog. |
