...
- Separate invention, diffusion, adoption lag, enforcement reach, state capacity, settlement structure, and economic complexity before broader generator changes.
- Current v0.11 slices add elite/median technology access, invention capacity, adoption-lag pressure, technology-access inequality diagnostics, and practical enforcement reach. Remaining work should continue with deeper state-capacity refinement and per-technology diffusion only after human source review.
...
Expected work:
- Expand the new `SmallBodyReservoir` records into dedicated viewer/export panels. System Viewer now has a compact overview/selected-belt readout, but TNO/Centaur/comet-feeding context still needs a first-class panel rather than only belt-anchored summary rows.
...
Continue with **Sentient populations, technology, and governance** after human review of the remaining source-note metadata. The small-body slice now has active source provenance, narrow generator retuning, diagnostic TNO subfamily metadata, first-class reservoir records, and compact System Viewer readouts, while dedicated reservoir panels, separate generated TNO population records, and survey-bias modeling remain deferred small-body follow-up.
