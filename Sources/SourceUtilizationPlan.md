...
Remaining work:
- Replace the diagnostic nuclear stellar component with calibrated nuclear cluster/disk structure only after source-specific values are human verified.
- Replace diagnostic baryon, cold-gas, hot-gas, stellar-halo, dark-halo, and local-density proxies with calibrated local mass-budget components when reviewed values are available.
- Replace diagnostic rotation-curve decomposition with calibrated potential/rotation behavior after source-specific values and downstream effects are reviewed.
