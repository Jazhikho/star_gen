...
Expected work:
- Expand the new `SmallBodyReservoir` records into dedicated viewer/export panels. System Viewer now has compact overview/selected-belt readouts and focusable large-object subentries, but TNO/Centaur/comet-feeding context still needs a first-class panel rather than only belt-anchored summary rows.
