...
**StarGen annotation:** Partly implemented as source-marked large-TNO/outer-reservoir support. StarGen now records this source on trans-Neptunian reservoir belts and first-class `SmallBodyReservoir` records, and uses it with `KavelaarsEtAl2023` for representative size-slope and diagnostic subfamily provenance. DES survey-bias modeling, calibrated counts, and separate orbital-family distributions remain follow-up.
...
**StarGen annotation:** Partly implemented as trans-Neptunian reservoir support. StarGen now shifts cold reservoirs toward Kuiper-belt analog distances, records this source on outer-belt reservoir, size-distribution, diagnostic subfamily provenance, and first-class `SmallBodyReservoir` records, and applies the public minor-body population slope to representative large bodies. Separate orbital-family distributions and calibrated population counts remain follow-up.
