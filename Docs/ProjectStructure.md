...
- `src/app/system_viewer/SystemInspectorPanel.cs`: System Viewer overview, selection, and reservoir-details readout, including source-facing `SmallBodyReservoir` family summaries grouped under their current asteroid-belt anchor, full reservoir source/span/status rows, neutral station/habitat settlement-readiness diagnostics, and focusable large-object subentries for inspectable belt bodies until export and calibrated population panels exist.
...
- `src/app/SystemGenerationScreen.cs`: system-studio controller mirrors the same `Space Opera` override controls, writes the user-tuned compatibility pressures into `SolarSystemSpec.UseCaseSettings`, and exposes asteroid-belt large-object display count/minimum-diameter controls for selecting inspectable belt bodies.
...
- `src/domain/galaxy/GalaxyMassComponentBudget.cs`: diagnostic-only galaxy mass-component budget carrying halo, dark matter, baryonic, stellar disk/spheroid/halo, nuclear stellar, cold gas, hot gas, baryon/gas fraction, local stellar-density, source/status metadata, and audit notes for future dynamics work without changing generation behavior.
- `src/domain/galaxy/GalaxyRotationCurveDiagnostic.cs`: diagnostic-only galaxy rotation-curve decomposition carrying inner/reference/outer velocity anchors, disk/spheroid/gas/dark-matter velocity contributions, curve-shape label, source/status metadata, and audit notes for future dynamics work without changing generation behavior.
- `src/domain/galaxy/GalaxyDynamicsDiagnostic.cs`: diagnostic-only galaxy dynamics surface carrying bar pattern speed, corotation radius, corotation/bar ratio, local mass-density and surface-density proxies, analog calibration mode, non-Milky-Way comparison status, source/status metadata, and audit notes.
- `src/domain/galaxy/GalaxyRealismProfile.cs`: derived galaxy realism profile carrying morphology, GHZ, metallicity, star-formation priors, source-marked Milky-Way structural schema fields, diagnostic mass components, diagnostic rotation, and final diagnostic dynamics/local-mass/comparison surfaces.
- `src/domain/galaxy/GalaxyRealismProfileBuilder.cs`: deterministic builder that resolves the scientific galaxy profile from user config and seed, including Bland-Hawthorn/Gerhard thin/thick disk, bar, solar-circle, velocity, stellar-mass, diagnostic component-budget fields, diagnostic rotation fields, and final diagnostic dynamics/local-mass/comparison fields.
...
- `Tests/Unit/TestGalacticScienceFutureBehavior.cs`: intentionally failing F2P future-behavior baseline for galactic science, covering the planned behavior mode and mode semantics, origin-context annotations, Studio/Viewer diagnostics and readout purity, non-Milky-Way calibration presets, local-space annotations, and overlay surfaces. It runs only through the explicit `f2p` headless filter.
