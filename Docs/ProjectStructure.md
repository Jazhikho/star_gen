...
- `src/domain/generation/parameters/LifeScienceReferenceCatalog.cs`: source registry and plain-language help content for the galaxy-studio life-potential models.
- `src/domain/population/LifePotentialModeling.cs`: life-model resolver that converts the selected biosphere or civilization model into concrete support, abiogenesis, complex-life, and sentience tuning.
- `Sources/Texts/LineweaverDavis2002.txt`, `Sources/Texts/SpiegelTurner2012.txt`, `Sources/Texts/ForganRice2010.txt`, `Sources/Texts/Mills2024.txt`, `Sources/Texts/Balbi2023.txt`: reviewed source notes added for the `0.8.9.0` life-model replacement covering optimistic biogenesis, cautionary abiogenesis interpretation, Rare Earth filtering, environmental windows, and oxygen bottlenecks for technospheres.
