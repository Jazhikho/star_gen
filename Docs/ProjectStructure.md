...
- `src/domain/galaxy/GalaxyRealismProfile.cs`: derived galaxy realism profile carrying morphology, GHZ, metallicity, star-formation priors, source-marked Milky-Way structural schema fields, the diagnostic galaxy mass-component budget, and the diagnostic rotation-curve surface.
- `src/domain/galaxy/GalaxyRealismProfileBuilder.cs`: deterministic builder that resolves the scientific galaxy profile from user config and seed, including Bland-Hawthorn/Gerhard thin/thick disk, bar, solar-circle, velocity, stellar-mass, diagnostic component-budget fields, and diagnostic rotation-curve fields for Milky-Way analog defaults.
