...
- `src/app/system_viewer/SystemInspectorPanel.cs`: System Viewer overview, selection, and reservoir-details readout, including source-facing `SmallBodyReservoir` family summaries grouped under their current asteroid-belt anchor, full reservoir source/span/status rows, station/habitat follow-up caveats, and focusable large-object subentries for inspectable belt bodies until export and calibrated population panels exist.
