...
| Bland-Hawthorn and Gerhard (2016), local shorthand `Bland2016` / `BlandHawthornGerhard2016` | Partly implemented as the active Milky-Way structural schema anchor. `GalaxyConfig`, `GalaxySpec`, and `GalaxyRealismProfile` now carry thin/thick disk scale lengths and heights, bar half-length, solar-circle radius, circular velocity at the solar circle, and stellar-mass scale; spiral density and bar-region classification use the new disk/bar geometry. `GalaxyMassComponentBudget`, `GalaxyRotationCurveDiagnostic`, and `GalaxyDynamicsDiagnostic` now add diagnostic-only halo/dark/baryonic/gas/local-density budgets, rotation-curve anchors, component velocity contributions, pattern speed, corotation, local mass-budget proxies, and non-Milky-Way comparison caveats. | The non-behavioral galaxy-hardening schema is now complete for the current pass. Still not a full Milky-Way simulator: calibrated nuclear cluster/disk structure, gravitational potential/orbit integration, source-fitted active local mass behavior, and family-specific non-Milky-Way comparison behavior remain follow-up. |
...
|---|---|---|
| Galaxy schema | Bland-Hawthorn and Gerhard (2016) is partly implemented for first-class Milky-Way structural schema fields: thin/thick disk dimensions, bar half-length, solar-circle radius, circular velocity, stellar mass, diagnostic mass-component budgets, diagnostic rotation-curve decomposition, diagnostic pattern/corotation/local-mass fields, and explicit non-Milky-Way comparison caveats. | Add calibrated nuclear cluster/disk structure, active gravitational-potential/local-mass behavior, orbit integration, and source-backed non-Milky-Way comparison behavior before claiming full galaxy-realism closure. |
...
| Small bodies / reservoirs | DeMeo and Carry (2014), Bauer et al. (2017), Kavelaars et al. (2023), Bernardinelli et al. (2022), and Raymond/Izidoro (2017) are partly implemented as active small-body reservoir proxies. Generated asteroids and belts now record asteroid taxonomy/composition source IDs; cold belts are labeled as trans-Neptunian reservoirs, placed farther outward than the old frost-line-adjacent belt, use a lower TNO-scale mass ceiling, record TNO/comet source provenance, and carry diagnostic cold classical, hot classical, resonant, scattered, Centaur, and comet-feeding subfamily metadata. Generated systems persist those families as first-class `SmallBodyReservoir` records with native-life absence and neutral station/habitat readiness diagnostics, System Viewer lists reservoir-family weights in the overview, selected-belt readout, and dedicated reservoir-details section with source IDs/radial span/diagnostic status/settlement-readiness, and inspectable large belt objects now use diameter-based semantics with selectable body records selected from the largest eligible candidate-pool objects; System Studio defaults remain 10 objects at 500 km minimum but expose both knobs. `Lamy2004` is rejected/replaced as active support by Bauer; `NapierEtAl2023` remains context. | Add actual station/habitat records, reservoir export surfaces, separate orbital-family distributions, and survey-bias-aware counts/calibrated size-luminosity functions. |
...
| Technology/governance | Hamilton (2020), Bettencourt (2007), Arvidsson (2023), Knez (2023), Balland (2022), Chacua (2024), Van Kleef (2023), and the pending-human-review Chowdhury/Comin/Mestieri/Stokey notes are now partly implemented through sentient-world source controls and diagnostics. `SentientWorldProfileBuilder` separates elite/median technology access, invention capacity, adoption-lag pressure, technology-access inequality, state capacity, legal reach, enforcement reach, legitimacy, and economic complexity as human-audit-required proxies. | Restriction pressure still needs deeper source review; technology diffusion is still aggregate, not per-technology or time-series; Chowdhury, Comin/Mestieri, and Stokey source-note metadata needs human cleanup before stronger claims. |

## Global F2P hardening surface 2026-05-13

`Docs/ScienceHardeningCompletionPlan.md` now defines the remaining completion-critical hardening families and their tradeoffs: planet-level HZ mass correction, alternative orbital-stability screens, moon-channel diagnostics, small-body population/export surfaces, galaxy-dynamics behavior gates, and human-audit-required sentient-population proxies. `ScienceHardeningEngineCatalog` registers those model families and alternatives so the explicit F2P harness can verify the plan is executable, not only prose. The first active diagnostic engines added in this pass are planet-mass-corrected Kopparapu 2014 HZ provenance and orbit-slot AMD/dynamical-packing/resonance diagnostics. These additions remain AI-assisted and pending human verification.
...
- Galaxy-realism follow-up for Bland-Hawthorn and Gerhard (2016): the non-behavioral structural, component-budget, rotation, dynamics, local-mass, and comparison-caveat schema is active; calibrated behavior, gravitational potential, orbit integration, local mass-budget effects, and broader galaxy-family comparison sources remain open before full generator-fit closure
