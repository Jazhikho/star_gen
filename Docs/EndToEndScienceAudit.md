...
| Small bodies / reservoirs | DeMeo and Carry (2014), Bauer et al. (2017), Kavelaars et al. (2023), Bernardinelli et al. (2022), and Raymond/Izidoro (2017) are partly implemented as active small-body reservoir proxies. Generated asteroids and belts now record asteroid taxonomy/composition source IDs; cold belts are labeled as trans-Neptunian reservoirs, placed farther outward than the old frost-line-adjacent belt, use a lower TNO-scale mass ceiling, record TNO/comet source provenance, and carry diagnostic cold classical, hot classical, resonant, scattered, Centaur, and comet-feeding subfamily metadata. Generated systems persist those families as first-class `SmallBodyReservoir` records, System Viewer lists reservoir-family weights in the overview, selected-belt readout, and dedicated reservoir-details section with source IDs/radial span/diagnostic status, and inspectable large belt objects now use diameter-based semantics with selectable body records selected from the largest eligible candidate-pool objects; System Studio defaults remain 10 objects at 500 km minimum but expose both knobs. `Lamy2004` is rejected/replaced as active support by Bauer; `NapierEtAl2023` remains context. | Add reservoir export surfaces, separate orbital-family distributions, neutral belt-station/habitat settlement records, and survey-bias-aware counts/calibrated size-luminosity functions. |
