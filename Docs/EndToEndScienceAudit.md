...
| Small bodies / reservoirs | DeMeo and Carry (2014), Bauer et al. (2017), Kavelaars et al. (2023), Bernardinelli et al. (2022), and Raymond/Izidoro (2017) are partly implemented as active small-body reservoir proxies. Generated asteroids and belts now record asteroid taxonomy/composition source IDs; cold belts are labeled as trans-Neptunian reservoirs, placed farther outward than the old frost-line-adjacent belt, use a lower TNO-scale mass ceiling, and record TNO/comet source provenance. `Lamy2004` is rejected/replaced as active support by Bauer; `NapierEtAl2023` remains human-verification pending context. | Split the single cold-reservoir proxy into cold classical, hot classical, resonant, scattered, Centaur, and comet-feeding populations; add survey-bias-aware counts and calibrated size/luminosity functions after human verification. |
