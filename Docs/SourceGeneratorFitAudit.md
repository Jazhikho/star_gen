...
|---|---|---|---|---|
| `BauerEtAl2017` | `CometGenerator`, `CometSpec`, `SystemAsteroidGenerator` | `partial fit` | `CometGenerator` has Jupiter-family vs long-period families, activity states, low albedo, low density, bare-nucleus handling, and orbital eccentricity/inclination distinctions. It does not yet use Bauer's JFC median radius near 0.6 km, NIC larger-radius prior, active-fraction metadata, Damocloid/HTC distinction, or power-law population slope for cometary bodies. Current default comet radii (`1-12 km`, or `7-30 km` for large) are much larger than the source-note JFC median and should be treated as StarGen tuning until retuned. | Add a comet-nucleus model pass or document the current ranges as non-calibrated. Consider `ActiveFraction`, `TisserandClass`, and Bauer-style radius/albedo sampling. |
