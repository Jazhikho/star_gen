...
## Population framework (merged to master)

The **population framework** is on **master**. It provides a detailed model for planets: native populations (and their history) and colonies.

**Purpose:** A robust **population framework for planets** that can represent both native populations (and their history) and colonies.
...
**Population framework stages (complete):** Stage 1: Planet Profile model (ClimateZone, BiomeType, ResourceType, HabitabilityCategory, PlanetProfile, ProfileCalculations). Stage 2: ColonySuitability + SuitabilityCalculator. Stage 3: History (HistoryEvent, PopulationHistory, HistoryGenerator). Stage 4: NativePopulation + NativePopulationGenerator. Stage 5: Colony (ColonyType, NativeRelation, Colony, ColonyGenerator). Stage 6: Integration boundary — **PlanetPopulationData** and **PopulationGenerator**. All in `src/domain/population/` with unit tests in `Tests/Unit/Population/`; RunTestsHeadless and TestScene include population tests; PopulationDeps preloads types.

---

## Outposts and space stations (merged to master)

**Outposts and space stations** are on **master**. They extend the population framework with orbital and deep-space habitats.
...
- **Outposts** — Population 10,000 or less. Placed in bridge systems (waypoints, trade routes, choke points) or other locales (mining, science, spy base, etc.).
- **Space stations** — Can act as outposts but may support city-sized populations; over native-populated or colony worlds, or in resource-rich systems. StationGenerator, StationPlacementRules, StationSpec; OutpostAuthority; OLIGARCHIC regime. Prototype: `src/app/prototypes/StationGeneratorPrototype.tscn`.
...
All feature work is on **master**. Population, outposts and space stations, jump lanes, object rendering (Phase 5–style shaders, population in viewer), and Phase 8 viewer display (SystemDisplayLayout, OrbitRenderer) are merged. No active feature branches; main development continues on master. Phase 8 constraint stages (SystemConstraints, stellar locks, resonances, UI) are still planned.
