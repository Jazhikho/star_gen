...
This work is developed on the **population** branch. It is intentionally somewhat separate from the main roadmap above; it will be integrated into the main branch once the framework is stable and aligned with the rest of the codebase.

**Purpose:** A detailed, robust **population framework for planets** that can represent both native populations (and their history) and colonies.

**Scope:**
•	**Native populations** — Indigenous or evolved populations on a world: origin, demographics, cultural/technological development, and history over time.
•	**Colonies** — Settled populations: founding origin (world or faction), founding date, relationship to native populations (if any), and ongoing demographic and political evolution.
•	**History** — The framework supports historical layers (events, migrations, conflicts, technological shifts) so that both native and colonial populations can have a coherent, queryable past.

**Relationship to main roadmap:** The population framework does not replace or block the current phases (object → system → galaxy). It is built so that it can eventually attach to planetary bodies generated by the existing system; for now it is developed and tested in isolation.

**Population framework stages (see Docs/PopulationFrameworkPlan.md):** Stages 1–6 are complete. Stage 1: Planet Profile model (ClimateZone, BiomeType, ResourceType, HabitabilityCategory, PlanetProfile, ProfileCalculations). Stage 2: ColonySuitability + SuitabilityCalculator. Stage 3: History (HistoryEvent, PopulationHistory, HistoryGenerator). Stage 4: NativePopulation + NativePopulationGenerator. Stage 5: Colony (ColonyType, NativeRelation, Colony, ColonyGenerator). Stage 6: Integration boundary — **PlanetPopulationData** (container with get_total_population, get_dominant_population, get_political_situation, serialization) and **PopulationGenerator** (generate(body, context, spec), generate_from_profile(profile, spec), generate_profile_only). All in `src/domain/population/` with unit tests in `Tests/Unit/Population/`; RunTestsHeadless and TestScene include population tests; PopulationDeps preloads types.

**Testing:**
•	The **normal test suite** (unit + integration for the main app) continues to run unchanged and must stay green.
•	Population framework code is **runnable separately** for live or exploratory tests (e.g. dedicated scenes or runners for population logic), so that main CI and local runs are not blocked by in-progress population work.
•	When the framework is merged, it will include appropriate unit and integration tests and will be part of the standard test suite.
