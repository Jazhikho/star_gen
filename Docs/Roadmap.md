...
**Stage 1 (Planet Profile Model):** ✅ Complete — ClimateZone, BiomeType, ResourceType, HabitabilityCategory, PlanetProfile in `src/domain/population/`; unit tests in `Tests/Unit/Population/`; RunTestsHeadless includes population tests; PopulationDeps preloads enums. Serialization supports JSON string keys. ProfileCalculations (body → Profile) is Stage 2.
